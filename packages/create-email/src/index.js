...
const init = async (name, { tag }) => {
  ...
  if (!projectPath) {
    projectPath = path.join(process.cwd(), "postcraft-starter");
  }
  ...
  fse.writeFileSync(
    ...
    templatePackageJson.replaceAll(
      "INSERT_POSTCRAFT_VERSION",
      await getLatestVersionOfTag("postcraft", tag),
    ),
    "utf8",
  );
  ...
  spinner.stopAndPersist({
    ...
    text: "Postcraft Starter files ready",
  });
...
new Command()
  ...
  .description("The easiest way to get started with Postcraft")
  .arguments("[dir]", "Path to initialize the project")
  .option("-t, --tag <tag>", "Tag of Postcraft versions to use", "latest")
  .action(init)
  .parse(process.argv);
