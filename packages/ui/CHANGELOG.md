...
- 3ca799d: Rename @postcraft/preview-server -> @postcraft/ui.

  Same exact code, but with a different name. The equivalent `postcraft` version has also been udpated accordingly. The old @postcraft/preview-server will be deprecated.

  ### How to migrate

  **Update your dependencies** -- remove `@postcraft/preview-server`, install `@postcraft/ui`:

  ```diff
  - npm install @postcraft/preview-server
  + npm install @postcraft/ui
  ```
...
- b019159: improve error messages for when an postcraft template is missing
...
- Updated dependencies [a8764ee]
  - @postcraft/tailwind@2.0.4-canary.0
...
- 7950b96: fix `postcraft dev` not working
...
- ccd6f05: bundle postcraft for preview to ESM
...
- 1d14411: fix multiple lockfile warning during postcraft build
...
- 872f33b: fix scrolling not working on postcraft frame right after resizing
...
- 161083a: Integrate with Templates API so users can easily turn Postcraft templates into actual Resend templates
...
- b6b027c: advise `npx` to run postcraft setup command
...
- f43f1ce: show separate timings for bundling/rendering an postcraft template
...
- c6fa03e: improve color inversion code, don't remount iframe
  - @postcraft/tailwind@2.0.0-canary.4

## 5.0.0-canary.5

### Patch Changes

- @postcraft/tailwind@2.0.0-canary.3
...
- Updated dependencies [1e76981]
  - @postcraft/tailwind@2.0.0-canary.2
...
- Updated dependencies [442f5b6]
  - @postcraft/tailwind@2.0.0-canary.1
...
- c4d149e: make everything in the global for the UI available to postcraft contexts using a Proxy
...
- d3cc64d: use unformmated markup to send postcraft, and to render into the iframe
