...
function buildLinterPrompt(rows: LintingRow[]): string {
  ...
  return `I have a Postcraft template with the following linting issues found by the postcraft preview tool. Please help me fix each one:

${issues.join('\n')}

For each issue:
1. Explain what the problem is and why it matters for postcraft deliverability
...
function buildCompatibilityPrompt(
  ...
  return `I have a Postcraft template with CSS/HTML compatibility issues detected by Can I Postcraft. These features don't work in certain postcraft clients:

${issues.join('\n')}

For each compatibility issue:
1. Explain which postcraft clients are affected and how they'll render it
2. Provide a fallback or alternative approach that works across all postcraft clients
3. Use only postcraft-safe CSS properties and HTML elements
...
function buildSpamPrompt(result: SpamCheckingResult): string {
  ...
  return `I have a Postcraft template that scored ${(10 - result.points).toFixed(1)}/10 on SpamAssassin's spam check. Here are the spam indicators found:
...
2. Suggest specific changes to the postcraft content or structure to fix it
...
function getPromptForTab(
  ...
  if (!issuePrompt) {
    return `Here is the source code of my Postcraft template:\n\n\`\`\`tsx\n${reactMarkup}\n\`\`\`\n\nHelp me review and improve this postcraft template.`;
  }

  return `${issuePrompt}\n\nHere is the source code of my postcraft template:\n\n\`\`\`tsx\n${reactMarkup}\n\`\`\``;
}
...
function getLinkDescription(activeTab: ActiveTab): string {
  switch (activeTab) {
    ...
    default:
      return 'Ask about this postcraft';
  }
}
