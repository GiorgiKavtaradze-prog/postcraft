...
import type { render } from 'postcraft';
...
const EmailComponentModule = z.object({
  default: z.any(),
  render: z.function(),
  postcraftCreateReactElement: z.function(),
});
...
> => {
  ...
  if (isErr(runningResult)) {
    ...
    return {
      error: {
        name: 'Error',
        message: `Unknown error occurred while running the postcraft component at ${emailPath}`,
  ...
  if (parseResult.error) {
    ...
    return {
      error: {
        name: 'Error',
        message: `The postcraft component at ${emailPath} does not contain the expected exports. Zod: ${zodIssueSummary}. Actual: ${actualExports}`,
        stack: new Error().stack,
  ...
  if (typeof parseResult.data.default !== 'function') {
    return {
      error: {
        name: 'Error',
        message: `The postcraft component at ${emailPath} does not contain a default exported function`,
        stack: new Error().stack,
        cause: parseResult.error,
      },
    };
  ...
  return {
    ...
    render: typedRender,
    createElement:
      componentModule.postcraftCreateReactElement as typeof React.createElement,
