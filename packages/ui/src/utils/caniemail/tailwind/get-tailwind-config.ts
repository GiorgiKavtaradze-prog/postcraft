...
import type { TailwindConfig } from 'postcraft';
...
const postcraftTailwindConfigInternal = ${configSourceValue};`,
          sourcePath,
        );
      } catch (exception) {
...
const getConfigFromCode = async (
  ...
  const configBuildResult = await esbuild.build({
    ...
    stdin: {
      contents: `${code} 
export { postcraftTailwindConfigInternal };`,
      sourcefile: filepath,
      loader: 'tsx',
  ...
  if (
    ...
    'postcraftTailwindConfigInternal' in configModule.value
  ) {
    return configModule.value
      .postcraftTailwindConfigInternal as TailwindConfig;
  }
