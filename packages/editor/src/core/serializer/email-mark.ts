...
export interface EmailMarkConfig<Options, Storage>
  ...
  renderToPostcraft: MarkRendererComponent;
}

type ConfigParameter<Options, Storage> = Partial<
  Omit<EmailMarkConfig<Options, Storage>, 'renderToPostcraft'>
> &
  Pick<EmailMarkConfig<Options, Storage>, 'renderToPostcraft'> &
  ThisType<{
    name: string;
    options: Options;
    storage: Storage;
...
> extends Mark<Options, Storage> {
  ...
  static from<O, S>(
    ...
    customMark.config = { ...mark.config, renderToPostcraft };
    return customMark;
  }
