...
export interface EmailNodeConfig<Options, Storage>
  ...
  renderToPostcraft: NodeRendererComponent;
}

type ConfigParameter<Options, Storage> = Partial<
  Omit<EmailNodeConfig<Options, Storage>, 'renderToPostcraft'>
> &
  Pick<EmailNodeConfig<Options, Storage>, 'renderToPostcraft'> &
  ThisType<{
    name: string;
    options: Options;
    storage: Storage;
...
> extends Node<Options, Storage> {
  ...
  static from<O, S>(
    ...
    customNode.config = { ...node.config, renderToPostcraft };
    return customNode;
  }
