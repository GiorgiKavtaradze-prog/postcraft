...
import { pretty, render, toPlainText } from "postcraft";
...
interface ComposePostcraftResult {
  html: string;
  text: string;
}

export const composePostcraft = async ({
  editor,
  preview,
}: {
  editor: Editor;
  preview?: string;
}): Promise<ComposePostcraftResult> => {
  ...
  function parseContent(content: JSONContent[] | undefined, depth = 0) {
    ...
    return content.map((node: JSONContent, index: number) => {
      ...
      const NodeComponent = emailNode.config.renderToPostcraft;
      ...
      if (node.marks) {
        for (const mark of sortMarksBySchema(node.marks, editor.schema)) {
          ...
          if (emailMark instanceof EmailMark) {
            const MarkComponent = emailMark.config.renderToPostcraft;
            const markStyle =
