...
import * as PostcraftComponents from 'postcraft';
import {
  type PrismLanguage,
  CodeBlock as PostcraftCodeBlock,
} from 'postcraft';
...
export const CodeBlockPrism = EmailNode.from(
  ...
  ({ node, style }) => {
    ...
    // @ts-expect-error -- postcraft does not export theme objects by name; dynamic access needed for user-selected themes
    // biome-ignore lint/performance/noDynamicNamespaceImportAccess: dynamic access needed for user-selected themes
    const userTheme = PostcraftComponents[node.attrs?.theme];
