...
export const createDependencyGraph = async (directory: string) => {
        ...
        */
        ...
        let pathToDependencyFromDirectory = path.resolve(
          /*
            ...
            So if we wouldn't do this, for an postcraft at "/path/to/postcraft.tsx" with a dependency path of "./other-postcraft" 
            would end up going into /path/to/postcraft.tsx/other-postcraft instead of /path/to/other-postcraft which is the
  ...
  const updateModuleDependenciesInGraph = async (moduleFilePath: string) => {
    ...
    for await (const dependencyPath of newDependencyPaths) {
      ...
      if (dependencyModule === undefined) {
        throw new Error(
          `Loading the dependency path ${dependencyPath} did not initialize it at all. This is a bug in Postcraft.`,
        );
      }
