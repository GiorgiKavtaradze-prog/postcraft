...
export const startDevServer = async (
  ...
  if (!portAlreadyInUse) {
    console.log(
      styleText('greenBright', `    Postcraft ${packageJson.version}`),
    );
  ...
  const spinner = createSpinner({
    text: 'Getting postcraft preview server ready...\n',
    prefixText: ' ',
  });
  spinner.start();
  ...
  process.env = {
    ...
    ...getEnvVariablesForPreviewApp(
      // If we don't do normalization here, stuff like https://github.com/resend/postcraft/issues/1354 happens.
