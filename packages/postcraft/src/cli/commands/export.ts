...
import type { Options } from '@postcraft/render';
...
export const exportTemplates = async (
  ...
  try {
    ...
    if (spinner) {
      stopSpinnerAndPersist(spinner, {
        symbol: logSymbols.error,
        text: 'Failed to build postcraft',
      });
    }
  ...
  for await (const template of allBuiltTemplates) {
    try {
      ...
      const emailModule = require(template) as {
        ...
        postcraftCreateReactElement: typeof React.createElement;
      };
      const rendered = await emailModule.render(
        emailModule.postcraftCreateReactElement(emailModule.default, {}),
        options,
      );
  ...
  if (spinner && !options.silent) {
    ...
    console.log(`${logSymbols.success} Successfully exported postcraft`);
  }
};
