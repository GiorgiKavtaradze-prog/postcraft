...
- 599b8c5: fix type issues in starter template and in postcraft

## 6.0.0

### Major Changes

- d0a7a52: Move all components and utilities into the `postcraft` package
  ...
  We're going to deprecate all packages except `@postcraft/render` and `@postcraft/ui`, and they will not be updated anymore.

  ### Breaking change

  Imports from `@postcraft/components`, `@postcraft/render`, or individual component packages (e.g. `@postcraft/button`) are no longer the recommended path and they will all be deprecated with the exception of `@postcraft/render` and `@postcraft/editor`, and `render` will remain exported from `postcraft`. Consumers should import everything from `postcraft`.

  ### Why

  Having separate packages for components (`@postcraft/components`), and the CLI (`postcraft`) created unnecessary confusion, and a maintenance burden for us.

  ### How to migrate
  1. Remove `@postcraft/components`:

     ```diff
     npm remove @postcraft/components
     ```

  2. Update `postcraft`, and move it over to `dependencies`:

     ```diff
     npm install postcraft@latest
     ```

  3. **Update your imports**:

     ```diff
     - import { Button, Html, Head, render } from "@postcraft/components";
     + import { Button, Html, Head, render } from "postcraft";
     ```
...
- Updated dependencies [7fc539d]
  - @postcraft/render@2.0.7-canary.0
...
- d0a7a52: Move all components and utilities into the `postcraft` package
  ...
  We're going to deprecate all packages except `@postcraft/render` and `@postcraft/preview-server`, and they will not be updated anymore.
  ...
  ### How to migrate
  1. **Update your dependencies** -- remove `@postcraft/components`, keep `postcraft`:

     ```diff
     - npm install @postcraft/components postcraft @postcraft/preview-server
     + npm install postcraft @postcraft/preview-server
     ```
  ...
  3. The `@postcraft/preview-server` and `@postcraft/editor` packages are not included in `postcraft`
...
- 11f56c5: fix RESEND_API_KEY being overwritten in postcraft preview
...
- 7c18bd3: don't require installing @postcraft/preview-server in the project, pack it into `$HOME/.postcraft`
...
- 800c960: `postcraft build` always failing
...
- 3c2aa37: use turbo for `postcraft build`
...
- 95c7417: Dark mode switcher emulating postcraft client color inversion
...
- ef77691: fix path resolution done wrong breaking `postcraft dev` on Windows

## 4.1.0

### Minor Changes

- abf8599: use a separate package for storing the preview server (@postcraft/preview-server)

### Patch Changes

- e173b44: Use the same version for the preview-server and postcraft
...
- 40fb596: Use the same version for the preview-server and postcraft

## 4.1.0-canary.8

### Minor Changes

- ed2f46a: use a separate package for storing the preview server (@postcraft/preview-server)
...
- b947f98: Pre-render postcraft templates on hover
...
- aae2f59: Ensure dependencies outside postcraft directory are completely resolved
- b4b2373: Fix non-postcraft files being rendered during hot reloading
...
- dbf3a64: Add error message for when an postcraft template does not have a default export

## 4.1.0-canary.0

### Minor Changes

- 4a0d4e3: Theme switcher for postcraft template
...
- a07eebf: Pre-render postcraft templates on hover
...
- 1a17219: fix improper `require` in `postcraft export`
...
- 4c7f597: fix `postcraft dev` not working with `traversal` error
...
- cd02449: Ensure dependencies outside postcraft directory are completely resolved
...
- ab70556: Fix non-postcraft files being rendered during hot reloading
- 9c9aa5d: Add error message for when an postcraft template does not have a default export
...
- 403e415: Fix `deno` not working as an option for `postcraft build`
...
- 382c305: Fixes active state for root postcraft templats on file tree
...
- 98b434c: Fixes active state for root postcraft templats on file tree
...
- 7337d04: Fix postcraft being re-rendered each time there is navigation in the preview server
...
- 7eb68af: Fix NODE_ENV for postcraft as "PRODUCTION" instead of "DEVELOPMENT"
- 76183e0: Update socket.io and socket.io-client
- f295614: Add AbortSignal, Event and EventTarget
- bb79540: Add missing timers/promises Node module for `postcraft dev`
...
- 59e27bc: Add missing timers/promises Node module for `postcraft dev`
...
- 1af53fb: Fix NODE_ENV for postcraft as "PRODUCTION" instead of "DEVELOPMENT"
...
- 1a47335: Use a built version of preview app when running `postcraft dev`

### Patch Changes

- 19cab7b: Fixes decorators causing dependency tree babel parsing to fail
- 14b7d1d: update socket.io/socket.io-client to 4.7.5
- b68e166: Fix sharp warning when running `postcraft dev`
- 3caaf53: Updated peer dependencies to allow for React 19 release candidated and React 19 itself
- 9a5fd78: fix postcraft template preview failing with postcraft having spaces
...
- f9483ec: Deprecated `renderAsync` and made `render` itself always async
  ...
  See https://github.com/resend/postcraft/discussions/1144 for more info.
...
- b87cbbf: Fix sharp warning when running `postcraft dev`

## 3.0.0-canary.0

### Major Changes

- f552226: Use a built version of preview app when running `postcraft dev`

### Patch Changes

- 2799bb4: fix postcraft template preview failing with postcraft having spaces
...
- Fixes live refresh not working with files outside `postcraft` directory
