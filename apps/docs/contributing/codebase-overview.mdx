...
description: 'An overview of the Postcraft codebase'
icon: 'folder-tree'
---

We've created this guide to help new contributors understand and navigate the Postcraft codebase.

## Top-level directories

After cloning the [Postcraft repository](https://github.com/resend/postcraft) you will see a few root-level directories. Here's a brief overview of each:
...
<table>
  ...
  <tbody>
    <tr>
      <td>
[apps](https://github.com/resend/postcraft/tree/canary/apps)
      </td>
      <td>
        Here you can find all of the apps related to our online presence, like:
        - this documentation (under [apps/docs](https://github.com/resend/postcraft/tree/canary/apps/docs)),
        - the demo postcraft we have on [demo.postcraft](https://demo.postcraft/preview/Community/notifications/vercel-invite-user)
          (under [apps/demo](https://github.com/resend/postcraft/tree/canary/apps/demo))
        - the Next app we have for our landing page on [postcraft](https://postcraft) (under [apps/web](https://github.com/resend/postcraft/tree/canary/apps/web))
      </td>
    </tr>
    <tr>
      <td>
[benchmarks](https://github.com/resend/postcraft/tree/canary/benchmarks)
      </td>
      <td>
        ...
        For example, see the [Improved Performance for Tailwind Postcraft](https://resend.com/blog/improved-performance-for-tailwind-postcraft) benchmark.
      </td>
    </tr>
    <tr>
      <td>
        [packages](https://github.com/resend/postcraft/tree/canary/packages)
      </td>
      <td>
...
<Note>
  Feel free to [open a discussion](https://github.com/resend/postcraft/discussions/new?category=ideas) if you have suggestions on how to better structure these packages to make them more manageable and approachable.
</Note>

## Multiple packages

The postcraft repository is a [pnpm monorepo](https://pnpm.io/next/workspaces), which means it contains
multiple packages.
...
- [postcraft](https://github.com/resend/postcraft/tree/canary/packages/postcraft)
    - <span className="text-xs">The package for our [postcraft CLI](/cli), and for all of our components</span>
- [@postcraft/editor](https://github.com/resend/postcraft/tree/canary/packages/editor)
    - <span className="text-xs">The embeddable visual editor for composing Postcraft templates</span>
- [@postcraft/render](https://github.com/resend/postcraft/tree/canary/packages/render)
    - <span className="text-xs">The [render](/utilities/render) utility. Can also be imported from `postcraft`.</span>
- [@postcraft/ui](https://github.com/resend/postcraft/tree/canary/packages/ui)
- [create-postcraft](https://github.com/resend/postcraft/tree/canary/packages/create-postcraft)
...
The `build`, `dev`, and `start` commands all depend on the user first installing `@postcraft/ui`.
Locally installing the preview server also includes all the required dependencies so you can run the necessary commands.

[nypm](https://www.npmjs.com/package/nypm) and [jiti](https://www.npmjs.com/package/jiti) work together to first ensure `@postcraft/ui` is installed and then to import it into the CLI.
...
the updated files to the Preview Server using [socket.io](https://socket.io/). Other details, like the path to the user's postcraft directory, are handled through environment variables.

### The Preview Server

The Preview Server is a Next.js app that uses `esbuild` at runtime to bundle the
user's postcraft templates and then renders them using the [render](/utilities/render) utility.
...
<Note>
The `@postcraft/render` package's `renderAsync` does a fair bit of magic to simulate `edge` and other environments that are not supported by `happy-dom`. For this use case, we override the [environment on a per-file basis](https://vitest.dev/guide/environment#environments-for-specific-files) for its tests
</Note>
...
We use [tsup](https://github.com/egoist/tsup) to build most packages. (The only exception for this is the `@postcraft/tailwind` package which currently uses `vite` due to a few issues with `tsup` and `tailwindcss`'s bundling.) For help building packages, see our [Development workflow guide](/contributing/development-workflow/4-building).
