...
StarterKit and replaces most nodes with postcraft-aware versions that know how to serialize
to Postcraft components.
...
`StarterKit` includes two structural extensions that shape how postcraft content is edited
and exported:

- `Container` keeps top-level content inside a dedicated postcraft content wrapper and
  serializes it with Postcraft's `Container` component.
...
<Accordion title="Block Nodes">
  ...
  | `Container` | Main postcraft content wrapper |
  ...
  | `Button` | Postcraft button element |
  | `PreviewText` | Postcraft preview text (shown in inbox list) |
</Accordion>
