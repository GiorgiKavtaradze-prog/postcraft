...
description: "Create custom postcraft-compatible editor nodes and marks."
icon: "wrench"
---
...
method: `renderToPostcraft()`. Use `parseHTML()` to recognize pasted or imported HTML,
`renderHTML()` to control how the node appears in the editor, and `renderToPostcraft()` to
control how it is exported by `composePostcraft`.
...
extension with postcraft serialization support. This is useful when you want to reuse a community
TipTap extension and add postcraft export support without rewriting it.
...
<Info>
  ...
  [`EmailNode`](/editor/api-reference/postcraft-node) and [`EmailMark`](/editor/api-reference/postcraft-mark)
  reference pages.
</Info>
...
<CardGroup cols={2}>
  <Card title="Custom Extensions" icon="code" href="https://postcraft/editor/examples/custom-extensions">
