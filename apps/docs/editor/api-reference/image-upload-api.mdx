...
The `useEditorImage` hook from `@postcraft/editor/plugins` creates the image
extension used for paste, drop, file-picker, and slash-command uploads.

```tsx
import { StarterKit } from '@postcraft/editor/extensions';
import { useEditorImage } from '@postcraft/editor/plugins';
...
```tsx
import { imageSlashCommand } from '@postcraft/editor/plugins';
...
If you're using [`EmailEditor`](/editor/api-reference/postcraft-editor), you can
