...
```tsx
import { BubbleMenu, bubbleMenuTriggers, useBubbleMenuContext } from '@postcraft/editor/ui';
```
...
A typical postcraft editor uses multiple bubble menus together. Use `hideWhenActiveNodes` and `hideWhenActiveMarks`
...
```tsx
import { BubbleMenu, bubbleMenuTriggers } from '@postcraft/editor/ui';
...
```tsx
import { useBubbleMenuContext } from '@postcraft/editor/ui';
