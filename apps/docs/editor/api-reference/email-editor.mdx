...
`EmailEditor` is the highest-level way to embed the Postcraft editor. It wraps
...
<Tip>
  `EmailEditor` automatically imports `@postcraft/editor/themes/default.css`,
...
>
  Serializes the current document to postcraft-ready HTML and plain text in one
  call.
</ResponseField>
...
export function MyEditor() {
  ...
  return (
    <div>
      <EmailEditor
        ref={editorRef}
        content="<h1>Welcome</h1><p>Edit this postcraft.</p>"
        theme="basic"
        onReady={(ref) => {
