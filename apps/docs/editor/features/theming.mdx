...
description: "Apply and customize visual themes for postcraft output."
icon: "palette"
---
...
Themes are CSS-in-JS style objects that map to postcraft component types (headings, paragraphs,
links, buttons, etc.). Each theme defines a set of `React.CSSProperties` for every supported
component.

During [`composePostcraft`](/editor/api-reference/compose-postcraft), the `EmailTheming`
...
Postcraft components as `style` attributes. This is necessary because postcraft clients don't
reliably support `<style>` tags or external stylesheets.

The resolved styles are passed to each node's `renderToPostcraft()` method via the `style`
...
```tsx
import { StarterKit } from '@postcraft/editor/extensions';
import { EmailTheming, extendTheme } from '@postcraft/editor/plugins';
import { EditorProvider } from '@tiptap/react';
...
```tsx
import { createTheme } from '@postcraft/editor/plugins';
...
Themes define styles for the following postcraft components:

| Component | Description |
|-----------|-------------|
| `reset` | CSS reset styles |
| `body` | Postcraft body wrapper |
...
When `EmailTheming` is in your extensions array, the `composePostcraft` function automatically
...
See [Postcraft Export](/editor/features/postcraft-export) for more details on the serialization pipeline.
...
<CardGroup cols={2}>
  <Card title="Postcraft Theming" icon="code" href="https://postcraft/editor/examples/postcraft-theming">
    Basic/Minimal/Custom theme toggle with live preview.
  </Card>
  <Card title="Custom Themes" icon="palette" href="https://postcraft/editor/examples/custom-theme">
