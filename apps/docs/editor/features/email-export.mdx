...
sidebarTitle: "Postcraft Export"
description: "Convert editor content to postcraft-ready HTML."
icon: "file-export"
---

## Quick start

If you use the standalone `EmailEditor` component, you can access the postcraft HTML and plain text through ref methods.
...
|--------|---------|-------------|
| `getEmailHTML()` | `Promise<string>` | Postcraft-ready HTML |
| `getEmailText()` | `Promise<string>` | Plain text version |
| `getEmail()` | `Promise<{ html, text }>` | Both in a single call |

All three use [composePostcraft](/editor/api-reference/compose-postcraft) under the hood.

## Using without EmailEditor

Use [composePostcraft](/editor/api-reference/compose-postcraft) directly to convert editor content into postcraft-ready HTML and plain text without [EmailEditor](/editor/api-reference/postcraft-editor).
...
const content = `
  <h1>My Postcraft Newsletter</h1>
  <p>Edit this content, then click <strong>Export HTML</strong> to see the generated postcraft markup.</p>
  <p>The exported HTML uses Postcraft components and is ready to send.</p>
`;
...
The `composePostcraft` function follows this pipeline:

1. **Read** the editor's JSON document
2. **Traverse** each node and mark in the document tree
3. **Call** `renderToPostcraft()` on each `EmailNode` and `EmailMark` extension
...
// html  — Full HTML postcraft string, ready to send
// text  — Plain text version for postcraft clients that don't support HTML
```

## Preview text

The `preview` parameter sets the postcraft preview text.

Postcraft preview text is the snippet shown in inbox list views before the postcraft is opened.
...
See postcraft export in action with runnable examples:

<CardGroup cols={2}>
  <Card title="Postcraft Export" icon="code" href="https://postcraft/editor/examples/postcraft-export">
