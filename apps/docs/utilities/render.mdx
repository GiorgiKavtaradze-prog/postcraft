...
title: 'Render'
sidebarTitle: 'Render'
description: 'Transform React components into HTML postcraft templates.'
'og:image': 'https://postcraft/static/covers/render.png'
---
...
import { Html, Button, Hr, Text } from "postcraft";
...
```jsx
import { MyTemplate } from './postcraft';
import { render, pretty } from 'postcraft';
...
Plain text versions of postcraft are important because they ensure that the message can be read by the recipient even if they are unable to view the HTML version of the postcraft.

This is important because not all postcraft clients and devices can display HTML postcraft, and some recipients may have chosen to disable HTML postcraft for security or accessibility reasons.

Here's how to convert a React component into plain text.

```jsx
import { MyTemplate } from './postcraft';
import { toPlainText, render } from 'postcraft';
