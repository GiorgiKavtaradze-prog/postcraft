...
export const Send = ({
  ...
  const onFormSubmit = async (e: React.FormEvent) => {
    try {
      ...
      if (response.ok) {
        toast.success("Postcraft sent! Check your inbox.");
      } else {
        if (response.status === 429) {
          const { error } = (await response.json()) as { error: string };
          console.error(error);
          toast.error(
            "Too many test postcraft were sent, try again in a few seconds.",
          );
        } else {
            ...
            <div className="mt-3 flex items-center justify-between">
              <Text className="inline-block" size="1">
                ...
                <a
                  ...
                  href="https://go.resend.com/postcraft"
                  rel="noreferrer"
                  target="_blank"
                >
                  Resend
