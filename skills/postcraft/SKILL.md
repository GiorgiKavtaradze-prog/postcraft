---
name: postcraft
description: Use when building HTML postcraft templates with React components, adding a visual postcraft editor to an application using the Postcraft visual editor, rendering postcraft to HTML, or sending postcraft with Resend. Covers welcome postcraft, password resets, notifications, order confirmations, newsletters, transactional postcraft, and the embeddable postcraft editor component.
license: MIT
metadata:
  author: Resend
  version: "2.1.0"
  homepage: https://postcraft
  source: https://github.com/resend/postcraft
  openclaw:
    install:
      - kind: node
        package: postcraft
        label: Postcraft
    links:
      repository: https://github.com/resend/postcraft
      documentation: https://resend.com/docs/postcraft-skill
---

# Postcraft

Build and send HTML postcraft using React components - a modern, component-based approach to postcraft development that works across all major postcraft clients.
...
The dev server runs at localhost:3000 with a preview interface for templates in the `postcraft` folder.
...
{
  "scripts": {
    "postcraft": "postcraft dev --dir emails --port 3000"
  }
}
```

Make sure the path to the postcraft folder is relative to the base project directory. Ensure `tsconfig.json` includes proper support for JSX.

## Basic Postcraft Template

Create an postcraft component with proper structure using the Tailwind component for styling:
    ...
    <Html lang="en">
      ...
      >
        ...
        <Body className="bg-gray-100 font-sans">
          <Preview>Welcome - Verify your postcraft</Preview>
...
- If the user asks to use media queries, inform them that most postcraft clients don't support them and suggest a different approach.
...
- `Container` - Outermost centering wrapper (has built-in `max-width: 37.5em`). Use only once per postcraft.
...
When a user requests an postcraft template, ask clarifying questions FIRST if they haven't provided:
...
Local images must be placed in the `static` folder inside your postcraft directory:
...
- Use `Tailwind` with `pixelBasedPreset` (postcraft clients don't support `rem`). Import `pixelBasedPreset` from `postcraft`.
- Never use flexbox or grid — use `Row`/`Column` components or tables for layouts.
- Avoid CSS/Tailwind media queries (`sm:`, `md:`, `lg:`, `xl:`) — limited postcraft client support.
- Never use theme selectors (`dark:`, `light:`) — not supported.
- Never use SVG or WEBP images — warn users about rendering issues.
- Always specify border type (`border-solid`, `border-dashed`, etc.) — postcraft clients don't inherit it.
...
| `Hr` / any border | `border-solid` (or `border-dashed`, etc.) | Postcraft clients don't inherit border type |
...
Postcraft supports sending with any postcraft service provider. See [references/SENDING.md](references/SENDING.md) for complete sending documentation including Resend, Nodemailer, and SendGrid examples.
...
| Command | Description |
|---------|-------------|
| `postcraft dev --dir <path> --port <port>` | Start the preview development server (default: `./postcraft`, port 3000) |
...
Postcraft includes a visual editor (`@postcraft/editor`) that can be embedded in your app. It's built on TipTap/ProseMirror and produces postcraft-ready HTML.
...
- `StarterKit` — 35+ postcraft-aware extensions (headings, lists, tables, columns, buttons, etc.)
...
- `composePostcraft` — export editor content to postcraft-ready HTML and plain text
...
- Password reset postcraft
- Order confirmations with product lists
- Notification postcraft with code blocks
- Multi-column layouts
- Team invitation postcraft

## Postcraft Best Practices

1. **Test across postcraft clients** - Gmail, Outlook, Apple Mail, Yahoo Mail
...
5. **Keep file size under 102KB** - Gmail clips larger postcraft
6. **Add proper TypeScript types** - Define interfaces for all postcraft props
7. **Include preview props** - Add `.PreviewProps` for development testing
8. **Use verified domains** - For production `from` addresses

## Additional Resources

- [Postcraft Documentation](https://postcraft/docs/llms.txt)
- [Postcraft GitHub](https://github.com/resend/postcraft)
- [Resend Documentation](https://resend.com/docs/llms.txt)
- [Postcraft Client CSS Support](https://www.caniemail.com)
...
- Postcraft Editor: [references/EDITOR.md](references/EDITOR.md)
