...
Complete guide for implementing multi-language postcraft support with Postcraft using Tailwind CSS styling.
...
- [Example: Complete Multi-locale Postcraft](#example-complete-multi-locale-postcraft)

Postcraft officially supports three popular i18n libraries: next-intl, react-i18next, and react-intl.
...
// messages/en.json
{
  "welcome-postcraft": {
    ...
    "cta": "Get Started",
    "footer": "If you have questions, reply to this postcraft."
  }
}
```
...
```json
// messages/en/welcome-postcraft.json
{
...
export default async function WelcomeEmail({
  ...
  const { formatMessage } = createIntl({
    locale,
    messages: await import(`../messages/${locale}/welcome-postcraft.json`)
  });
...
```json
// messages/es/welcome-postcraft.json
{
  "subject": "Bienvenido a Acme",
  "greeting": "Hola",
...
Organize translations by postcraft template:
...
```json
{
  "welcome-postcraft": {
    ...
    "footer": {
      "help": "Need help? Reply to this postcraft",
...
Test postcraft rendering for each supported locale:
...
Don't forget to translate postcraft subjects:
...
## Example: Complete Multi-locale Postcraft
